...
import { ArrowLeft, Save, Syringe, Loader2, AlertCircle, RefreshCw, PawPrint, Calendar, User, Package } from "lucide-react";
...
export default function VaccinationRecordsPage() {
  ...
  const fetchPendingRecords = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Lỗi khi lấy danh sách hồ sơ tiêm phòng');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchCustomerPackages = async (maKhachHang) => {
    ...
    try {
      const response = await api.get(`/vaccinations/customer/${maKhachHang}/subscriptions`);
      if (response.data.success) {
        // Chỉ lấy các gói đang hoạt động
        const activePackages = (response.data.data || []).filter(p => p.TrangThai === 'Đang hoạt động');
  ...
  const fetchAllVaccines = async () => {
    ...
    try {
      ...
      if (response.data) {
        const vaccineList = Array.isArray(response.data) ? response.data : (response.data.data || []);
  ...
  const fetchPackageVaccines = async (maGoiDK) => {
    setVaccinesLoading(true);
    try {
      const response = await api.get(`/vaccinations/packages/${maGoiDK}/vaccines`);
      if (response.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('❌ Lỗi: ' + (err.response?.data?.message || 'Không thể cập nhật hồ sơ tiêm phòng'));
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 mt-1">Chọn hồ sơ để cập nhật thông tin tiêm phòng</p>
          </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Click vào hồ sơ để chọn vaccine tiêm</CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
              ...
              <div className="text-center py-12 text-gray-500">
                ...
                <p className="text-sm mt-1">Tất cả hồ sơ tiêm phòng đã được chọn vaccine</p>
              </div>
            ) : (
        ...
        <Dialog open={showForm} onOpenChange={setShowForm}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Chọn vaccine cho {selectedRecord?.tenKhachHang} - {selectedRecord?.tenThuCung}
              </DialogDescription>
            </DialogHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {packagesLoading ? (
                  <div className="text-center py-4">
                    ...
                    <p className="text-sm text-gray-500 mt-2">Đang tải danh sách gói...</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    ...
                    <Card
                      className={`cursor-pointer transition-all p-3 ${
                        selectedPackage === null || selectedPackage === 'none'
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`h-4 w-4 rounded-full border-2 flex items-center justify-center ${
                          selectedPackage === null || selectedPackage === 'none' ? "border-green-600" : "border-gray-300"
                        }`}>
                          {(selectedPackage === null || selectedPackage === 'none') && (
                        ...
                        <div className="flex-1">
                          <p className="font-semibold text-gray-700">
                            {packages.length === 0 ? "Khách hàng chưa đăng ký gói nào" : "Không sử dụng gói (Hiển thị tất cả vaccine)"}
                          </p>
                    ...
                    {packages.map((pkg) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`h-4 w-4 rounded-full border-2 flex items-center justify-center ${
                            selectedPackage === pkg.MaGoiDK ? "border-green-600" : "border-gray-300"
                          }`}>
                            {selectedPackage === pkg.MaGoiDK && (
                              <div className="h-2 w-2 rounded-full bg-green-600" />
                            )}
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-gray-500">
                              Mã: {pkg.MaGoiDK} | Ưu đãi: {pkg.UuDai}% | HSD: {new Date(pkg.ThoiGianKetThuc).toLocaleDateString('vi-VN')}
                            </p>
                          </div>
              ...
              <div className="space-y-2">
                ...
                {vaccinesLoading ? (
                  <div className="text-center py-4">
                    <Loader2 className="h-6 w-6 mx-auto text-green-500 animate-spin" />
                    <p className="text-sm text-gray-500 mt-2">Đang tải danh sách vaccine...</p>
                  </div>
                ) : vaccines.length === 0 ? (
                  <div className="text-center py-4 text-gray-500">
                    <p>Không có vaccine nào {selectedPackage ? 'trong gói này' : 'khả dụng'}</p>
                  </div>
                ) : (
                  <div className="grid gap-2 max-h-60 overflow-y-auto">
                    {vaccines.map((vaccine) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`h-4 w-4 rounded-full border-2 flex items-center justify-center ${
                            selectedVaccine === vaccine.MaVacXin ? "border-green-600" : "border-gray-300"
                          }`}>
                            {selectedVaccine === vaccine.MaVacXin && (
                              <div className="h-2 w-2 rounded-full bg-green-600" />
                            )}
                          ...
                          <div className="flex-1">
                            <p className="font-semibold">{vaccine.TenVacXin}</p>
                            <p className="text-xs text-gray-500">
                              Mã: {vaccine.MaVacXin} | 
                              {vaccine.GiaSauUuDai !== undefined ? (
                                <> Giá gốc: <s>{vaccine.GiaGoc?.toLocaleString('vi-VN')}đ</s> → Giá ưu đãi: <span className="text-green-600 font-semibold">{vaccine.GiaSauUuDai?.toLocaleString('vi-VN')}đ</span></>
                              ) : (
                                <> Giá: {vaccine.GiaTien?.toLocaleString('vi-VN')}đ</>
                              )}
                            </p>
                          </div>
                        </div>
