...
export default function BranchesPage() {
  ...
  const checkIsOpen = (tgMoCua, tgDongCua) => {
    ...
    const parseTime = (timeStr) => {
      ...
      if (timeStr.hours !== undefined) return timeStr.hours * 60 + (timeStr.minutes || 0);
      return null;
    };
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4">
                <div className={`h-16 w-16 rounded-2xl ${currentService.bgColor} flex items-center justify-center shadow-sm`}>
                  <ServiceIcon className={`h-8 w-8 ${currentService.textColor}`} />
                </div>
                <div>
                  ...
                  <p className="text-slate-500 mt-2 font-medium">
                    {service === "products" ? "Khám phá danh mục sản phẩm tại các chi nhánh" : "Đặt lịch hẹn chăm sóc với các chuyên gia hàng đầu"}
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="container mx-auto px-6 py-16">
        {error ? (
          <Card className="p-16 text-center border-dashed border-2 rounded-[2.5rem]">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-2">Đã xảy ra lỗi</h2>
            <p className="text-slate-500 mb-8 max-w-sm mx-auto">{error}</p>
            <Button onClick={() => fetchBranches(1)} className="bg-slate-900 hover:bg-slate-800 text-white px-8 rounded-xl h-12">Thử lại ngay</Button>
          </Card>
        ) : loading ? (
          <div className="flex flex-col items-center justify-center py-24 space-y-4">
            <Loader2 className="h-12 w-12 animate-spin text-blue-600" />
            <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Đang tìm kiếm chi nhánh...</p>
          </div>
        ) : branches.length > 0 ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {branches.map((branch) => (
                <Card
                  ...
                  className="group border-slate-100 shadow-xl shadow-slate-100/50 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 rounded-[2.5rem] overflow-hidden flex flex-col"
                >
                  <CardHeader className="p-8 pb-4">
                    <div className="flex items-start justify-between mb-4">
                      <div className="h-12 w-12 bg-slate-50 rounded-xl flex items-center justify-center group-hover:bg-blue-50 transition-colors">
                        <MapPin className="h-6 w-6 text-slate-400 group-hover:text-blue-600 transition-colors" />
                      </div>
                      <Badge
                        variant="secondary"
                      ...
                      >
                        {checkIsOpen(branch.TGMoCua, branch.TGDongCua) ? "Đang mở cửa" : "Hiện đóng cửa"}
                      </Badge>
                    </div>
                    ...
                    <div className="space-y-3 mt-6">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-bold text-slate-700">{branch.SDT}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-semibold text-slate-600">
                           {(() => {
                              const formatTime = (time) => {
                                ...
                                if (time.hours !== undefined) return `${String(time.hours).padStart(2, "0")}:${String(time.minutes || 0).padStart(2, "0")}`;
                                return time.toString().substring(0, 5);
                              };
                              const openTime = formatTime(branch.TGMoCua);
                              const closeTime = formatTime(branch.TGDongCua);
                              return openTime && closeTime ? `${openTime} - ${closeTime}` : "Thông tin đang cập nhật";
                           })()}
                        </span>
                      </div>
                    </div>
                  ...
                  <CardContent className="p-8 pt-auto mt-auto">
                    ...
                    >
                      {service === "products" ? "Xem cửa hàng" : "Chọn chi nhánh"}
          ...
          <Card className="p-20 text-center border-dashed border-2 rounded-[3rem] bg-slate-50/50">
            ...
            <h3 className="text-2xl font-bold text-slate-900 mb-2">Không tìm thấy chi nhánh</h3>
            <p className="text-slate-500 mb-8 font-medium">Hiện tại không có chi nhánh nào hỗ trợ dịch vụ này.</p>
            <Link to="/customer">
              <Button className="bg-slate-900 hover:bg-slate-800 text-white rounded-xl h-12 px-8">Quay về trang chủ</Button>
            </Link>
          </Card>
        )}
      </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-xl p-0 overflow-hidden rounded-[2.5rem] border-none shadow-2xl">
          <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-8 text-white">
            <DialogHeader>
              <DialogTitle className="text-2xl font-black">Đặt lịch hẹn</DialogTitle>
            ...
            <div className="flex items-center gap-3 mt-6">
              <div className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${bookingStep >= 1 ? "bg-white" : "bg-white/20"}`} />
              <div className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${bookingStep >= 2 ? "bg-white" : "bg-white/20"}`} />
            </div>
          </div>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                {selectedBranch && (
                  <div className="p-4 bg-blue-50/50 rounded-2xl border border-blue-100 flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-slate-900 text-sm">{selectedBranch.TenChiNhanh}</p>
                      <p className="text-xs text-slate-500 font-medium line-clamp-1">{selectedBranch.DiaChi}</p>
                    </div>
                  </div>
                )}

                <div className="space-y-3">
                  <Label htmlFor="date" className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Thời gian mong muốn</Label>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="flex items-center justify-between px-1">
                  <div className="flex items-center gap-2 text-slate-900">
                    ...
                    <span className="font-bold text-sm">Ngày: {new Date(appointmentDate).toLocaleDateString('vi-VN')}</span>
                  </div>
                  <Button 
                    variant="ghost" 
                    size="sm" 
                ...
                <div className="space-y-4">
                  <Label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Chuyên gia phụ trách</Label>
                  ...
                  {loadingDoctors ? (
                    <div className="flex flex-col items-center justify-center py-12 space-y-4 bg-slate-50 rounded-[2rem] border border-dashed border-slate-200">
                      ...
                      <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest">Đang tìm bác sĩ rảnh...</p>
                    </div>
                  ) : availableDoctors.length > 0 ? (
                    <div className="grid gap-3 max-h-72 overflow-y-auto pr-2 thin-scrollbar">
                      {availableDoctors.map((doctor) => {
                        const isAvailable = doctor.TrangThai === "Rảnh";
                        const isSelected = selectedDoctor?.MaNhanVien === doctor.MaNhanVien;
                        ...
                        const formatTime = (time) => {
                          ...
                          if (time instanceof Date) return `${String(time.getUTCHours()).padStart(2, "0")}:${String(time.getUTCMinutes()).padStart(2, "0")}`;
                          if (time && typeof time === "object" && "hours" in time) return `${String(time.hours).padStart(2, "0")}:${String(time.minutes || 0).padStart(2, "0")}`;
                          return String(time);
                        };
                        ...
                        return (
                          <div
                            ...
                            className={`
                              ...
                              ${isSelected ? "border-blue-600 bg-blue-50/50" : isAvailable ? "border-slate-100 bg-white hover:border-blue-200 hover:bg-slate-50" : "border-slate-50 bg-slate-50/50 opacity-50 cursor-not-allowed"}
                            `}
                          >
                            <div className="flex items-center gap-4">
                              <div className={`h-11 w-11 rounded-xl flex items-center justify-center font-bold text-sm ${isSelected ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-500"}`}>
                                {doctor.HoTen.charAt(0)}
                              </div>
                              <div>
                                <p className={`font-bold text-sm ${isSelected ? "text-blue-700" : "text-slate-900"}`}>{doctor.HoTen}</p>
                                {doctor.GioLamViec && (
                                  <p className="text-[11px] text-slate-400 font-medium">
                                    Ca trực: {formatTime(doctor.GioLamViec.BatDau)} - {formatTime(doctor.GioLamViec.KetThuc)}
                                  </p>
                                )}
                              </div>
                            </div>
                            ...
                            {isSelected ? (
                              ...
                              <Badge variant="secondary" className="bg-slate-100 text-slate-400 text-[9px] uppercase font-bold">Bận</Badge>
                            )}
                          </div>
                        );
                      })}
                    ...
                    <div className="p-10 text-center text-slate-400 border-2 border-dashed border-slate-100 rounded-[2rem] bg-slate-50/50">
                      ...
                      <p className="font-bold text-xs uppercase tracking-wider">Không có bác sĩ sẵn sàng</p>
                      <p className="text-[10px] mt-1">Vui lòng chọn ngày khác hoặc chi nhánh khác</p>
                    </div>
                  )}
                </div>
              </div>
