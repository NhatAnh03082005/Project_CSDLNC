...
import { Heart, ArrowLeft, ShoppingBag, Plus, Minus, Loader2, Sparkles, Package, ShieldCheck, Truck, Zap, Star, Shield, Info, ArrowRight, Check } from "lucide-react";
...
export default function ProductDetailPage() {
  ...
  const getProductImage = (name, type) => {
    ...
    if (n.includes("sữa tắm")) return "https://www.phukienthucungdep.com/upload/images/sua-tam-cho-meo/sua-tam-cho-cho-long-trang1.jpg";
    if (n.includes("lược") || n.includes("bàn chải")) return "https://product.hstatic.net/200000264739/product/luoc_chai_long_co_nut_bam_cleanpet_s_cho_cho_meo_1_1e1f60bbbc7e43be891c1549a8808aea_master.jpg";
    if (n.includes("khăn lau")) return "https://down-vn.img.susercontent.com/file/vn-11134207-7ras8-m2i9xs63va9ec4.webp";
    if (n.includes("kìm bấm móng")) return "https://images.unsplash.com/photo-1516733968668-dbdce39c46ef?auto=format&fit=crop&q=80&w=800";

    // Thức ăn khô & Hạt
    if (n.includes("royal canin") || n.includes("hạt") || n.includes("cơm")) {
      if (n.includes("mèo")) return "https://images.unsplash.com/photo-1589924691106-073b19f55363?auto=format&fit=crop&q=80&w=800";
    ...
    if (n.includes("bánh thưởng") || n.includes("xương gặm") || n.includes("training")) {
    ...
    // Phụ kiện đeo & Dắt
    if (n.includes("vòng cổ") || n.includes("dây dắt") || n.includes("phản quang")) {
    ...
    // Vệ sinh & Làm đẹp
    if (n.includes("sữa tắm") || n.includes("lược") || n.includes("kìm bấm móng") || n.includes("khăn lau")) {
    ...
    // Mặc định theo loại
    if (type === "ThucAn") return "https://images.unsplash.com/photo-1568640347023-a616a30bc3bd?auto=format&fit=crop&q=80&w=800";
    if (type === "Thuoc") return "https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?auto=format&fit=crop&q=80&w=800";
    if (type === "PhuKien") return "https://images.unsplash.com/photo-1541591047357-9191ff29f9e6?auto=format&fit=crop&q=80&w=800";
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] relative overflow-hidden">
      ...
      <header className="sticky top-0 z-50 w-full border-b border-slate-100 bg-white/80 backdrop-blur-md">
        <div className="container mx-auto flex h-20 items-center justify-between px-6">
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <span className="text-2xl font-black text-slate-800 tracking-tight">PetCare</span>
          </Link>
      ...
      <div className="container mx-auto px-6 py-12 max-w-7xl relative z-10">
        <Link to={`/products-list?branch=${maChiNhanh}`} className="inline-flex items-center text-slate-500 hover:text-orange-600 font-bold text-sm mb-10 transition-colors group">
        ...
        {error ? (
          <div className="bg-white rounded-[3rem] border-2 border-dashed border-red-200 p-20 text-center shadow-sm max-w-3xl mx-auto">
             ...
             <Link to={`/products-list?branch=${maChiNhanh}`}>
                <Button className="h-14 px-12 rounded-2xl bg-slate-900 font-bold">Quay lại ngay</Button>
             </Link>
          </div>
        ) : loading ? (
          <div className="flex flex-col items-center justify-center py-40">
            ...
            <p className="mt-6 text-slate-400 font-black uppercase tracking-widest text-xs animate-pulse">Đang nạp dữ liệu sản phẩm...</p>
          </div>
        ) : product ? (
          <div className="grid lg:grid-cols-12 gap-12 items-start">
            {/* Left: Product Media */}
            <div className="lg:col-span-7 space-y-8">
               <div className="relative aspect-[4/3] rounded-[3.5rem] overflow-hidden bg-white shadow-2xl border border-slate-100 group">
                  <img 
                  ...
                  <div className="absolute top-8 left-8">
                     <Badge className={`${getProductTypeColor(product.loaiSanPham)} border-none rounded-2xl px-5 py-2 font-black text-xs uppercase tracking-widest shadow-xl backdrop-blur-md bg-opacity-90`}>
               ...
               <div className="grid grid-cols-3 gap-4">
                  {[
                    { icon: ShieldCheck, title: "Chất lượng", desc: "Đã kiểm duyệt", color: "text-emerald-500", bg: "bg-emerald-50" },
                    { icon: Truck, title: "Giao hàng", desc: "Hỏa tốc 2h", color: "text-orange-500", bg: "bg-orange-50" },
                    { icon: Star, title: "Đánh giá", desc: "4.9/5 điểm", color: "text-amber-500", bg: "bg-amber-50" }
                  ].map((feat, i) => (
                    <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col items-center text-center space-y-2 hover:shadow-md transition-shadow">
                       <div className={`h-12 w-12 ${feat.bg} rounded-xl flex items-center justify-center ${feat.color}`}>
                          <feat.icon className="h-6 w-6" />
                       </div>
                       <div>
                          <p className="text-xs font-black text-slate-900 uppercase tracking-tight">{feat.title}</p>
                          <p className="text-[10px] font-bold text-slate-400 uppercase">{feat.desc}</p>
                       </div>
                    </div>
                  ))}
               </div>
            ...
            <div className="lg:col-span-5 space-y-8">
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-4">
                   ...
                   <span className="text-slate-400 font-bold text-sm">(120+ nhận xét)</span>
                </div>
              </div>

              <div className="p-8 rounded-[3rem] bg-white border border-slate-100 shadow-xl space-y-8">
                <div className="flex items-end justify-between">
                    <div className="space-y-1">
                      <p className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Giá bán hiện tại</p>
                      <p className="text-4xl font-black text-orange-600 tracking-tight">{formatPrice(product.donGia)}</p>
                   </div>
                   <div className="text-right">
                      <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${product.soLuongTonKho > 0 ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
                         <div className={`h-1.5 w-1.5 rounded-full ${product.soLuongTonKho > 0 ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`} />
                         {product.soLuongTonKho > 0 ? 'Còn hàng' : 'Hết hàng'}
                      </div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">Tồn kho: {product.soLuongTonKho}</p>
                   </div>
                </div>

                <div className="space-y-4">
                   <Label className="text-xs font-black text-slate-900 uppercase tracking-[0.1em] pl-1">Chọn số lượng</Label>
                ...
                <div className="p-6 rounded-3xl bg-orange-50/50 border border-orange-100 space-y-4">
                   <div className="flex justify-between items-center text-sm">
                      <span className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">Đơn giá x{quantity}</span>
                      <span className="font-bold text-slate-700">{formatPrice(product.donGia)}</span>
                   </div>
                   <div className="flex justify-between items-center pt-2 border-t border-orange-100/50">
                      <span className="text-slate-800 font-black uppercase tracking-widest text-xs">Tổng thanh toán</span>
                      <span className="text-2xl font-black text-orange-700">{formatPrice(product.donGia * quantity)}</span>
                   </div>
                </div>
              ...
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm space-y-4">
                 <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">Mô tả sản phẩm</h3>
                 </div>
                 <p className="text-slate-500 leading-relaxed font-medium">
                    {product.moTa || "Sản phẩm chính hãng chất lượng cao, đã được kiểm duyệt an toàn cho thú cưng bởi đội ngũ chuyên gia PetCare."}
                 </p>
                 <div className="pt-4 border-t border-slate-50 flex items-center gap-4 text-[10px] font-bold text-slate-400 uppercase">
                    <p className="flex items-center gap-1.5"><Check className="h-3 w-3 text-emerald-500" /> Hoàn trả 7 ngày</p>
                    <p className="flex items-center gap-1.5"><Check className="h-3 w-3 text-emerald-500" /> BH chính hãng</p>
                 </div>
              </div>
            </div>
          </div>
