...
import { Plus, Calendar, Gift, Clock, Loader2, Sparkles, Syringe, ShieldCheck, Zap, ArrowRight, CheckCircle2, AlertCircle } from "lucide-react";
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div className="space-y-4">
              ...
              <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight">
                Gói <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-blue-600">Tiêm phòng</span>
              </h1>
              <p className="text-slate-500 text-lg max-w-lg leading-relaxed">
                Bảo vệ sức khỏe thú cưng của bạn một cách toàn diện với các gói ưu đãi tiêm chủng đặc biệt.
              </p>
            </div>

            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogTrigger asChild>
                <Button variant="premium" className="h-14 px-8 rounded-2xl shadow-xl shadow-emerald-200 hover:shadow-emerald-300 transition-all active:scale-[0.98] flex gap-3 text-base font-bold bg-gradient-to-r from-emerald-600 to-teal-500 border-none">
              ...
              <DialogContent className="sm:max-w-[700px] p-0 overflow-hidden border-none shadow-2xl rounded-[2.5rem]">
                <div className="bg-gradient-to-br from-emerald-600 via-teal-500 to-blue-600 px-8 py-12 text-white relative">
                  ...
                  <div className="flex items-center gap-6 relative z-10">
                    ...
                    <div>
                      <DialogTitle className="text-3xl font-black tracking-tight">Chọn gói Tiêm phòng</DialogTitle>
                ...
                <div className="p-8 space-y-4 max-h-[60vh] overflow-y-auto custom-scrollbar bg-slate-50/50">
                  {availablePackages.length === 0 ? (
                    <div className="text-center py-12 space-y-3">
                      ...
                      <p className="text-slate-500 font-medium">Hiện không có gói tiêm phòng nào khả dụng</p>
                    </div>
                  ) : (
                    availablePackages.map((pkg, index) => (
                      <div key={pkg.LoaiGoi || index} className="group relative bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-emerald-100/50 transition-all duration-300 flex items-center justify-between gap-4">
                ...
                <div className="p-8 pt-0 bg-slate-50/50">
                   <p className="text-center text-xs text-slate-400 font-medium">
                     * Các gói tiêm phòng được áp dụng ngay sau khi đăng ký thành công.
                   </p>
                </div>
              </DialogContent>
            </Dialog>
        ...
        {subscriptions.length === 0 ? (
          <div className="bg-white rounded-[3rem] border-2 border-dashed border-slate-200 p-20 text-center shadow-sm max-w-4xl mx-auto overflow-hidden relative">
            ...
            <div className="max-w-md mx-auto space-y-8 relative z-10">
              ...
              <div className="space-y-3">
                <h3 className="text-3xl font-black text-slate-900 tracking-tight">Chưa có gói tiêm phòng</h3>
                <p className="text-slate-500 text-lg leading-relaxed">
                  Đăng ký gói tiêm phòng để nhận ưu đãi giảm giá chi phí và giúp thú cưng của bạn luôn khỏe mạnh.
                </p>
              </div>
              <Button 
                variant="premium" 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {subscriptions.map((pkg) => {
              ...
              return (
                <div key={pkg.MaGoiDK} className="group relative bg-white rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-emerald-100/30 transition-all duration-500 overflow-hidden flex flex-col">
                  {/* Card Status Decoration */}
                  <div className={`absolute top-0 right-0 w-32 h-32 -mt-16 -mr-16 rounded-full blur-2xl opacity-40 transition-colors duration-500 ${isActive ? 'bg-emerald-400' : 'bg-slate-300'}`} />
                  ...
                  <div className="p-8 relative z-10 space-y-8">
                    <div className="flex items-start justify-between">
                      <div className="space-y-1">
                        ...
                        <p className="text-slate-400 font-bold text-xs uppercase tracking-widest pl-0.5">Mã: {pkg.MaGoiDK}</p>
                      </div>
                      <div className={`h-14 w-14 rounded-2xl flex items-center justify-center shadow-lg transition-transform duration-500 group-hover:scale-110 rotate-3 ${isActive ? 'bg-emerald-600 shadow-emerald-200' : 'bg-slate-400 shadow-slate-200'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-5 rounded-3xl bg-slate-50 border border-slate-100 space-y-2 group-hover:bg-white transition-colors">
                        ...
                        <p className="text-slate-700 font-extrabold text-lg">{pkg.duration}</p>
                      </div>
                      <div className="p-5 rounded-3xl bg-emerald-50/50 border border-emerald-100 space-y-2 group-hover:bg-white transition-colors">
                        ...
                        <p className="text-emerald-700 font-extrabold text-lg">{pkg.benefits.split(' ')[1]}</p>
                      </div>
                    </div>
                    ...
                    <div className="p-6 rounded-[2rem] bg-slate-900 text-white shadow-xl shadow-slate-200 group-hover:translate-y-[-4px] transition-transform duration-300">
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Thời gian sử dụng</span>
                        <div className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse" />
                      </div>
                      <div className="flex items-center justify-between gap-4">
                         <div className="space-y-1">
                           <p className="text-slate-400 text-[10px] font-bold uppercase">Bắt dầu</p>
                           <p className="font-bold text-sm tracking-tight">{pkg.startDate}</p>
                         </div>
                         <ArrowRight className="h-4 w-4 text-slate-600" />
                         <div className="space-y-1 text-right">
                           <p className="text-slate-400 text-[10px] font-bold uppercase">Kết thúc</p>
                           <p className="font-bold text-sm tracking-tight">{pkg.endDate}</p>
                         </div>
                      </div>
                    </div>
                  </div>
